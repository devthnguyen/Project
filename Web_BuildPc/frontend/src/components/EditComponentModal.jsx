...
import { updateComponent } from '../api/api';

const EditComponentModal = ({ isOpen, onClose, onUpdate, type, initialData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await updateComponent(type, formData._id, formData);
      onUpdate();
      onClose();
    } catch (err) {
      ...
      <div className="bg-primary-dark border border-neutral-700 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 flex justify-end space-x-3 mt-4 border-t border-neutral-700 pt-4">
            ...
            <button type="submit" disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded shadow-md transition-colors">
