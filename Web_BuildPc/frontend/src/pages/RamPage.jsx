...
const RamPage = () => {
  ...
  useEffect(() => {
    const fetchRAMs = async () => {
      setLoading(true);
      try {
        const data = await getRAMs(searchTerm);
        let sortedData = [...data];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Memory (RAM) Database</h1>
        <p className="mt-2 text-neutral-400">Upgrade your system's multitasking capabilities.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <SearchBar 
          ...
          placeholder="Search for RAM (e.g., 32GB, DDR5)..." 
        />
        <select 
          value={sortOrder} 
      ...
      {loading ? (
        <div className="py-10 text-center text-neutral-500">Loading RAM...</div>
      ) : (
          ...
          <table className="min-w-full divide-y divide-neutral-800 table-fixed">
            <thead className="bg-primary-dark">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-bold text-neutral-400 uppercase tracking-tight">Speed</th>
            ...
            <tbody className="bg-primary-dark divide-y divide-neutral-800/60">
              {rams.map((item) => (
                <tr key={item._id} className="hover:bg-primary-light/50 transition-colors">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-neutral-300 font-bold text-accent">
                    {item.capacity} GB
                  </td>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm text-white font-semibold text-right pr-6">
                    ${item.price}
                  </td>
                </tr>
              ))}
              {rams.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-neutral-500">No RAM found.</td>
                </tr>
              )}
            </tbody>
          </table>
